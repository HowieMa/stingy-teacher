...
def loss_fn_kd_reverse(outputs, labels, teacher_outputs, params):
    """
    ...
    value, index = torch.topk(teacher_outputs, k=teacher_outputs.shape[1], dim=1)
    reverse_value = torch.flip(value, [1])[:, :-1]
    reverse_index = index[:, 1:]

    teacher_outputs_reverse = torch.full(teacher_outputs.shape, float("-inf")).to(teacher_outputs.device)

    top_row = torch.tensor([[i] * 1 for i in range(index.shape[0])])
    reverse_row = torch.tensor(
        [[i] * (teacher_outputs_reverse.shape[1] - 1)
        for i in range(index.shape[0])]
    )
    teacher_outputs_reverse[top_row, index[:, 0].reshape((-1, 1))] = value[:, 0].reshape((-1, 1))
    teacher_outputs_reverse[reverse_row, reverse_index] = reverse_value

    KD_loss = nn.KLDivLoss(reduction='batchmean')(F.log_softmax(outputs/T, dim=1),
                             F.softmax(teacher_outputs_reverse/T, dim=1)) * (alpha * T * T) + \
...
# ************************** training function **************************
def train_epoch_kd(model, t_model, optim, loss_fn, data_loader, params):
    model.train()
    t_model.eval()
...
if __name__ == "__main__":
    ...
    checkpoint = torch.load(teacher_resume, map_location='cpu')
